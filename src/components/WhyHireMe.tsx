...
export default function WhyHireMe() {
  return (
    <section className="relative py-12 sm:py-16 lg:py-24">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(54,216,183,0.03),transparent_60%)]" />
          ...
          {reasons.map((reason, i) => (
            <motion.div key={reason.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.5, delay: i * 0.1 }} className="card flex items-start gap-4 p-5 rounded-xl group">
              <div className="mt-0.5 flex-shrink-0">
                <div className="card-icon !p-1.5 !rounded-full flex items-center justify-center" style={{ boxShadow: "0 0 8px rgba(124,92,252,0.15)" }}>
